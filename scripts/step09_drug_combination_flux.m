...
%  Calculates delta-flux for drug combination treatments (e.g., DOX+OLM,
%  CAR+TRS) using the same Noah linprog method as step03. Processes RNA-seq
%  files across 6 cell lines and computes MedMed, ByCell, and ByIndv
%  statistics.
%
%  Requirements:
%    - MATLAB
...
%    - HPC cluster recommended      (266 files x flux calculations)
%
%  Inputs:
...
%
%  Outputs:
%    - out/deltaFlux_stats.mat       — Combo statistics: PathStats*,
%                                      RxnStats*, RnaStats*
%                                      (MedMed, ByCell, ByIndv)
...
%% Initialize Output Tables (Reorganized for Combos)

% Create one row per combination pair
numCombos = length(DrugCombList);

% Output structure: {comboPair, cellLine, replicate} cells containing results
% We'll use a different approach: store results indexed by combo, cell, and replicate
...
% Results will be stored as: results.combo{a}.cell{cellIdx}.data
results.comboPair = comboPairStrs;
results.combo = cell(numCombos, 1);

for c = 1:numCombos
    results.combo{c}.cellLines = cell(length(uniqueCells), 1);
    for cl = 1:length(uniqueCells)
        results.combo{c}.cellLines{cl}.drug1_RNA = [];          % Drug1 RNA
        results.combo{c}.cellLines{cl}.drug2_RNA = [];          % Drug2 RNA
        results.combo{c}.cellLines{cl}.conv_FC = [];            % conv_FC
        results.combo{c}.cellLines{cl}.drug1_PathFlux = [];     % Drug1 pathway flux
        results.combo{c}.cellLines{cl}.drug1_RxnFlux = [];      % Drug1 reaction flux
        results.combo{c}.cellLines{cl}.ctrl1_PathFlux = [];     % ctrl1 pathway flux
        results.combo{c}.cellLines{cl}.ctrl1_RxnFlux = [];      % ctrl1 reaction flux
        results.combo{c}.cellLines{cl}.conv2_PathFlux = [];     % Conv pathway flux
        results.combo{c}.cellLines{cl}.conv2_RxnFlux = [];      % Conv reaction flux
        results.combo{c}.cellLines{cl}.ctrl2_PathFlux = [];     % ctrl2 pathway flux
        results.combo{c}.cellLines{cl}.ctrl2_RxnFlux = [];      % ctrl2 reaction flux
    end
end
...
for a = 1:numCombos
    ...
    %results.combo{a}.ctrl1_PathFlux = tempTable;
    results.combo{a}.ctrl1_RxnFlux = tempTable;
    %results.combo{a}.drug1_PathFlux = tempTable;
    results.combo{a}.drug1_RxnFlux= tempTable;
    %results.combo{a}.ctrl2_PathFlux = tempTable;
    results.combo{a}.ctrl2_RxnFlux = tempTable;
    %results.combo{a}.conv2_PathFlux = tempTable;
    results.combo{a}.conv2_RxnFlux = tempTable;
    %results.combo{a}.toxDrug_RNA = tempTable;
    %results.combo{a}.toxDrug_CTRL = tempTable;
    %results.combo{a}.conv_FC = tempTable;
    %results.combo{a}.helpDrug_CTRL = tempTable;
    ...
    for cl = 1:length(uniqueCells)
        ...
        conv_FC = helpDrug_RNA_table .* medToxDrug_RNA{:,:};

        % Calculate flux for Drug1 (single treatment)
        [ctrl1_PathFlux, ctrl1_RxnFlux, drug1_PathFlux, drug1_RxnFlux] = runSJ0_DEG_local(...
            Model, toxDrug_RNA_table, toxDrug_CTRL_table, cellLine, drug1);

        % Calculate flux for Convolution (combo estimate)
        [ctrl2_PathFlux, ctrl2_RxnFlux, conv2_PathFlux, conv2_RxnFlux] = runSJ0_DEG_local(...
            Model, conv_FC, toxDrug_CTRL_table, cellLine, [char(drug1) '+' char(drug2)]);

        %results.combo{a}.ctrl1_PathFlux{cellLine, string(drug1 + drug2)} = {ctrl1_PathFlux};
        results.combo{a}.ctrl1_RxnFlux{cellLine, string(drug1 + drug2)} = {ctrl1_RxnFlux};
        %results.combo{a}.drug1_PathFlux{cellLine, string(drug1 + drug2)} = {drug1_PathFlux};
        results.combo{a}.drug1_RxnFlux{cellLine, string(drug1 + drug2)} = {drug1_RxnFlux};
        %results.combo{a}.ctrl2_PathFlux{cellLine, string(drug1 + drug2)} = {ctrl2_PathFlux};
        results.combo{a}.ctrl2_RxnFlux{cellLine, string(drug1 + drug2)} = {ctrl2_RxnFlux};
        %results.combo{a}.conv2_PathFlux{cellLine, string(drug1 + drug2)} = {conv2_PathFlux};
        results.combo{a}.conv2_RxnFlux{cellLine, string(drug1 + drug2)} = {conv2_RxnFlux};
        rnaTable = table;
        rnaTable.GeneName = string(toxDrug_RNA_table.Properties.RowNames);
        rnaTable.Properties.RowNames = toxDrug_RNA_table.Properties.RowNames;
        %results.combo{a}.toxDrug_RNA{cellLine, string(drug1 + drug2)} = {[rnaTable, toxDrug_RNA_table]};
        rnaTable = table;
        rnaTable.GeneName = string(toxDrug_CTRL_table.Properties.RowNames);
        rnaTable.Properties.RowNames = toxDrug_CTRL_table.Properties.RowNames;
        %results.combo{a}.toxDrug_CTRL{cellLine, string(drug1 + drug2)} = {[rnaTable, toxDrug_CTRL_table]};
        rnaTable = table;
        rnaTable.GeneName = string(conv_FC.Properties.RowNames);
        rnaTable.Properties.RowNames = conv_FC.Properties.RowNames;
        %results.combo{a}.conv_FC{cellLine, string(drug1 + drug2)} = {[rnaTable, conv_FC]};
        rnaTable = table;
        rnaTable.GeneName = string(helpDrug_CTRL_table.Properties.RowNames);
        rnaTable.Properties.RowNames = helpDrug_CTRL_table.Properties.RowNames;
        %results.combo{a}.helpDrug_CTRL{cellLine, string(drug1 + drug2)} = {[rnaTable, helpDrug_CTRL_table]};

        disp(['  ✓ ' char(cellLine) ' processed']);

    end

    disp('Computing MedMed statistics...');

    drug1 = char(DrugCombList{a}{1});
    drug2 = char(DrugCombList{a}{2});

    % MedMed Statistics (Drug vs Mean DMSO across all conditions)
    %PathStatsMedMed_Drug = DToXs_statizizer_MedMed(results.combo{a}.drug1_PathFlux, results.combo{a}.ctrl1_PathFlux);
    RxnStatsMedMed_Drug = DToXs_statizizer_MedMed(results.combo{a}.drug1_RxnFlux, results.combo{a}.ctrl1_RxnFlux);
    %RnaStatsMedMed_Drug = DToXs_statizizer_MedMed(results.combo{a}.toxDrug_RNA, results.combo{a}.toxDrug_CTRL);
    disp('MedMed Stats Done');

    % ByCell Statistics (Drug vs DMSO within same cell line)
    %PathStatsByCell_Drug = DToXs_statizizer_ByCell(results.combo{a}.drug1_PathFlux, results.combo{a}.ctrl1_PathFlux);
    RxnStatsByCell_Drug = DToXs_statizizer_ByCell(results.combo{a}.drug1_RxnFlux, results.combo{a}.ctrl1_RxnFlux);
    %RnaStatsByCell_Drug = DToXs_statizizer_ByCell(results.combo{a}.toxDrug_RNA, results.combo{a}.toxDrug_CTRL);
    disp('ByCell Stats Done');

    % ByIndv Statistics (Individual sample comparisons)
    %PathStatsByIndv_Drug = DToXs_statizizer_ByIndv(results.combo{a}.drug1_PathFlux, results.combo{a}.ctrl1_PathFlux);
    RxnStatsByIndv_Drug = DToXs_statizizer_ByIndv(results.combo{a}.drug1_RxnFlux, results.combo{a}.ctrl1_RxnFlux);
    %RnaStatsByIndv_Drug = DToXs_statizizer_ByIndv(results.combo{a}.toxDrug_RNA, results.combo{a}.toxDrug_CTRL);
    disp('ByIndv Stats Done');

    % Store in results
    %results.combo{a}.PathStatsMedMed_Drug = PathStatsMedMed_Drug;
    results.combo{a}.RxnStatsMedMed_Drug = RxnStatsMedMed_Drug;
    %results.combo{a}.RnaStatsMedMed_Drug = RnaStatsMedMed_Drug;
    %results.combo{a}.PathStatsByCell_Drug = PathStatsByCell_Drug;
    results.combo{a}.RxnStatsByCell_Drug = RxnStatsByCell_Drug;
    %results.combo{a}.RnaStatsByCell_Drug = RnaStatsByCell_Drug;
    %results.combo{a}.PathStatsByIndv_Drug = PathStatsByIndv_Drug;
    results.combo{a}.RxnStatsByIndv_Drug = RxnStatsByIndv_Drug;
    %results.combo{a}.RnaStatsByIndv_Drug = RnaStatsByIndv_Drug;

    % MedMed Statistics (Drug vs Mean DMSO across all conditions)
    %PathStatsMedMed_Conv = DToXs_statizizer_MedMed(results.combo{a}.conv2_PathFlux, results.combo{a}.ctrl2_PathFlux);
    RxnStatsMedMed_Conv = DToXs_statizizer_MedMed(results.combo{a}.conv2_RxnFlux, results.combo{a}.ctrl2_RxnFlux);
    %RnaStatsMedMed_Conv = DToXs_statizizer_MedMed(results.combo{a}.conv_FC, results.combo{a}.toxDrug_CTRL);
    disp('MedMed Stats Done');

    % ByCell Statistics (Drug vs DMSO within same cell line)
    %PathStatsByCell_Conv = DToXs_statizizer_ByCell(results.combo{a}.conv2_PathFlux, results.combo{a}.ctrl2_PathFlux);
    RxnStatsByCell_Conv = DToXs_statizizer_ByCell(results.combo{a}.conv2_RxnFlux, results.combo{a}.ctrl2_RxnFlux);
    %RnaStatsByCell_Conv = DToXs_statizizer_ByCell(results.combo{a}.conv_FC, results.combo{a}.toxDrug_CTRL);
    disp('ByCell Stats Done');

    % ByIndv Statistics (Individual sample comparisons)
    %PathStatsByIndv_Conv = DToXs_statizizer_ByIndv(results.combo{a}.conv2_PathFlux, results.combo{a}.ctrl2_PathFlux);
    RxnStatsByIndv_Conv = DToXs_statizizer_ByIndv(results.combo{a}.conv2_RxnFlux, results.combo{a}.ctrl2_RxnFlux);
    %RnaStatsByIndv_Conv = DToXs_statizizer_ByIndv(results.combo{a}.conv_FC, results.combo{a}.toxDrug_CTRL);
    disp('ByIndv Stats Done');

    % Pack just the 6 RxnStats
    comboRxnStats.RxnMedMed_Drug = RxnStatsMedMed_Drug;
    comboRxnStats.RxnByCell_Drug = RxnStatsByCell_Drug;
    comboRxnStats.RxnByIndv_Drug = RxnStatsByIndv_Drug;
    comboRxnStats.RxnMedMed_Conv = RxnStatsMedMed_Conv;
    comboRxnStats.RxnByCell_Conv = RxnStatsByCell_Conv;
    comboRxnStats.RxnByIndv_Conv = RxnStatsByIndv_Conv;
    comboRxnStats.comboPair = comboPairStrs{a};

    disp('Saving RxnStats only...');
    if ~exist('out','dir'), mkdir('out'); end
    save(fullfile('out', sprintf('RxnStats_%s.mat', comboPairStrs{a})), 'comboRxnStats', '-v7.3');
    clear comboRxnStats;
...
%save(fullfile('out',"drugComboFluxALLANTH_stats.mat"), 'results', '-v7.3');


disp('Driver 9 completed successfully!');
